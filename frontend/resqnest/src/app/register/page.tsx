...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      email,
      password,
      role: role === "victim" ? "VICTIM" : "VOLUNTEER",
      fullName,
      phone,
      location,
      acceptTerms,
      skills: role === "volunteer" ? skills.join(", ") : undefined,
  ...
  const tabClass = (active: boolean) =>
    `flex-grow py-2 px-4 rounded-md text-body-sm transition-all text-center cursor-pointer ${
      active
  ...
  return (
    <>
      ...
      <main className="flex-grow flex items-center justify-center py-24 px-margin-mobile">
        ...
        <div className="w-full max-w-2xl bg-white rounded-xl shadow-md border border-outline-variant overflow-hidden">
          ...
          <form className="p-stack-lg space-y-stack-lg" onSubmit={handleSubmit}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SKILLS.map((skill) => {
                      ...
                      return (
                        <button
                          key={skill}
                          className={`px-3 py-1.5 rounded-full border border-secondary font-body-sm transition-all cursor-pointer ${
                            active
            ...
            {/* Error message */}
            {error && (
              <div className="flex items-center gap-2 p-4 rounded-lg bg-red-50 border border-red-200 text-red-800 font-body-sm text-body-sm">
            ...
            <div className="pt-4">
              <button
                className="w-full h-14 bg-primary-container text-white font-headline-md text-headline-md font-bold rounded-lg shadow-sm hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-70 cursor-pointer"
                type="submit"
                disabled={submitting}
              >
                {submitting ? (
      ...
      <footer className="bg-surface-container-low border-t border-outline-variant flex flex-col md:flex-row justify-between items-center py-stack-lg px-margin-mobile md:px-margin-desktop gap-stack-md">
        ...
        <p className="font-body-sm text-body-sm text-on-surface-variant">
          © 2024 ResQNest Emergency Management System. All rights reserved.
        </p>
      </footer>
    </>
  );
